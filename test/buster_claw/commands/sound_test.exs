defmodule BusterClaw.Commands.SoundTest do
  ...
  describe "dispatch by name" do
    ...
    test "the catalog advertises reads as safe and writers as restricted" do
      ...
      # The three that write. Restricted, and deliberately NOT gated: they write
      # a new source or a text index, and neither routes anything.
      for name <- ~w(sound_index_import sound_index_delete sound_assemble) do
        entry = Map.fetch!(catalog, name)
      ...
      assert Map.fetch!(catalog, "sound_probe").args["name"].required
