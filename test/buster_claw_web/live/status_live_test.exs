defmodule BusterClawWeb.StatusLiveTest do
  ...
  setup do
    ...
    # Stage 2 has its own seam and its own default: entering the tab now fires
    # a second real claude run, which must be stubbed as thoroughly as the first.
  ...
  describe "trading sub-tab" do
    test "renders the tab, the warning banner, and first-run setup", %{conn: conn} do
      ...
      # The banner names the write scope, now that the panel shows accounts the
      # agent may only read.
