...
The ACL artifacts `build.rs` writes — `gen/schemas/*.json` and
`permissions/autogenerated/*.toml` — are tracked, and the CI `rust` job fails if
a build changes them. Adding a command therefore shows its ACL entry in the same
diff; if the step goes red, run `cargo check` in `desktop/tauri` and commit
what it wrote.
