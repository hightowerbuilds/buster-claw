...
- `BusterClaw.Telephony` (+ `BusterClawWeb.PhoneLive`): BusterPhone — **intake only**. Inbound voicemail and SMS drained from a signed relay, transcripts, the local message archive, and the trusted-caller/PIN gates that decide what becomes queue work. There is no outbound path: the context sends no texts and places no calls, so it holds no carrier registration and no send caps.
- `BusterClaw.Notes`: the operator's Markdown vault under `notes/` — the Workspace page's Notes tab (it was Home's until 09-05), `note_*` commands, `[[wiki links]]` and backlinks. Distinct from `BusterClaw.Journal` (the Activity record) and from the Library.
