...
- **SSRF pinning falls back to validate-only in two narrow cases:** when DNS resolution is disabled by config (`:ssrf_resolve_dns` — off only in test), and inside the Playwright sidecar, which fetches with its own browser stack and does not use `URLGuard`'s pinned connection (it is Seatbelt-sandboxed and disabled by default in prod). ~~DNS rebinding (TOCTOU) in the SSRF guard~~ — **closed 07-14** by resolve-once-and-pin, exactly the fix this entry used to describe. The same change closed an unadvertised gap: Req 0.5 does not re-run request steps on redirect hops, so the old per-hop re-validation claim was aspirational — a redirect to an internal address was previously followed. Both paths are now covered by tests (`test/buster_claw/url_guard_test.exs`).
