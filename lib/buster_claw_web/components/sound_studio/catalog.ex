defmodule BusterClawWeb.SoundStudio.Catalog do
  ...
  **A `url` is added per kind, and `nil` is a real answer.** A mix has no URL
  because it is not a file the browser can play until it is rendered.
  """
  ...
  # A mix has to be rendered before anything can play it. `nil` on purpose
  # rather than absent, so `clip_src/2` below can match on the field without
  # knowing the kinds.
  ...
  @doc """
  The groups a clip may be added FROM: everything except mixes, because a mix is
  a list of references to these rather than raw material. Groups left empty by
  that filter drop out rather than rendering a headed but empty optgroup.

  It also used to reject `kind: :library` — the music library manager's row,
  which was a control rather than a source. That row was deleted on 08-16 and
  the reject went with it rather than being left as a guard against a kind
  nothing mints; a filter that can never match reads as protection and is not.
  """
  def addable_groups(groups) do
    Enum.reject(groups, &(&1.key == "mix" or &1.items == []))
  end
