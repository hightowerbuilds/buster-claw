defmodule BusterClawWeb.PhonePanels do
  @moduledoc """
  The Message Machine's three panels, as presentation-only function components:
  the **event log**, the **playback** stage over its keypad shader, and
  **contacts**.

  Split out of `BusterClawWeb.PhoneComponent` on 08-08, which was a 1,300-line
  module holding state, events and a 730-line template. Nothing here reads or
  writes state: each panel takes the assigns it needs and a `target`, so every
  binding reaches the stateful component rather than its host. That is the same
  division the trading components used before they were deleted — a stateful
  owner, and dumb panels that can be read on their own.

  **`target` rather than `@myself`.** A function component in another module has
  no `@myself`; the owner passes its own, and the panels thread it onto every
  binding explicitly. `LiveViewTest` resolves `phx-target` per element, so an
  ancestor's is not enough.
  """
