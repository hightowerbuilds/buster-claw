defmodule BusterClaw.Telephony.Twilio do
  @moduledoc """
  Twilio REST client for voicemail billing (`VOICEMAIL_COST_ROADMAP.md`). Twilio
  never sends price in a webhook; it lives on the REST resources and populates
  **asynchronously** (null right after the call, settled a bit later), so cost is
  a retryable back-fill, not a capture-at-drain value.

  This client only ever reads. BusterPhone is intake-only
  (`PHONE_INTAKE_ROADMAP.md`): outbound SMS and outbound calling were deleted
  08-18, along with the two kill switches, the Messaging Service, and the
  bridged-call pricing that used to be `cost_for/2`'s second clause. Nothing here
  creates a Twilio resource.

  `cost_for/2` prices one thing — a **voicemail** — from the **RecordingSid**
  every drained row already carries as its `twilio_sid`:
  ...
  That it fetches a `Calls` resource is not outbound code left behind: the
  inbound leg is part of what a voicemail costs. Deleting it by association would
  silently un-price every voicemail.
  ...
  Creds come from the Clinch with app env `:twilio` (`config/runtime.exs`) as
  fallback. `req_options` (Req.Test plugs) inject in tests.
  """
  ...
  @doc """
  The BusterPhone number as configured, for a UI that must name it before any
  traffic has arrived.

  `nil` when unset — which is the honest answer, and better than the alternative
  this replaced: deriving it from the most recent inbound row, which says nothing
  at all on a machine that has not been called yet.
  """
  def caller_id, do: phone_number()

  @doc """
  Total cost of one **voicemail**, from the RecordingSid the row already carries:
  the recording, its parent (inbound) call leg, and its transcription(s).
  Breakdown `%{call, recording, transcription}`.

  One shape, since intake-only leaves one thing that bills. Anything else is
  `{:error, :missing_sids}` — including a legacy `direction: "outbound"` call row,
  which stays in the ledger as a true record but no longer has a pricing route.
  ...
  # Live through the Clinch, env as fallback. There are no kill switches left to
  # read: they guarded the two outbound capabilities, and a deleted capability
  # needs no switch.
