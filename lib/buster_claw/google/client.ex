defmodule BusterClaw.Google.Client do
  ...
  def get_json(%Account{} = account, path, opts \\ []) do
    with {:ok, account, token} <- account_token(account, opts) do
      account
      |> request_json(:get, path, token, opts)
      |> maybe_refresh_and_retry(account, path, opts)
    end
  end
  ...
  defp maybe_refresh_and_retry({:error, {:unauthorized, _account_id}}, account, path, opts) do
    with {:ok, refreshed} <- OAuth.refresh_access_token(account, opts),
         {:ok, token} <- Account.decrypt(refreshed, :access_token) do
      request_json(refreshed, :get, path, token, opts)
    end
  end

  defp maybe_refresh_and_retry(result, _account, _path, _opts), do: result

  defp endpoint(path), do: @gmail_base_url <> "/" <> String.trim_leading(path, "/")
