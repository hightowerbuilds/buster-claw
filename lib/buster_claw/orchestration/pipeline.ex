defmodule BusterClaw.Orchestration.Pipeline do
  ...
  Beyond `noop`, the allowlisted Google Workspace commands are dispatched through
  the canonical `BusterClaw.Commands` surface as a `:trusted` caller — the task
  was pre-authorized when the user set it up in the wizard — so restricted actions
  like `gmail_send` run without a per-call confirmation. Account resolution and the
  Sentinel audit trail come for free from `Commands.call/3`.
  """
  require Logger

  alias BusterClaw.{Commands, Orchestration}

  @runner_supervisor BusterClaw.Orchestration.RunnerSupervisor

  # Deterministic GWS actions the wizard can schedule as pipeline tasks.
  @gws_commands ~w(gmail_sync google_calendar_sync gmail_search gmail_read
                   gmail_label_list gmail_draft_create gmail_send)
  ...
  defp execute(command, params) when command in @gws_commands do
    case Commands.call(command, params, caller: :trusted) do
      {:ok, result} -> {:ok, summarize(command, result)}
  ...
  # A short, bounded summary for the task's result_path; the full outcome is on
  # the Sentinel audit feed.
  defp summarize(command, result) do
    "#{command}: #{result |> inspect(limit: 5) |> String.slice(0, 280)}"
  end
