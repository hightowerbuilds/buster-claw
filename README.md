...
- **One command surface.** 219 commands across documents, browser, Google Workspace, integrations, finance, phone, notes, sketches, memory, skills, and orchestration — reachable from the CLI and an HTTP API, with per-caller trust tiers and an audit trail covering everything that changes state.
