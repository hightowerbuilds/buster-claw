...
You run **Claude Code, Codex, or OpenCode** — in the built-in terminal, or headlessly from the app's own Chat. The agent drives the app through one canonical command surface (203 commands) covering your browser, Gmail, Calendar, Drive, GitHub, and a durable work queue. Every command that changes something, every outbound send, and every untrusted fetch lands on an auditable security feed, and restricted actions are refused outright for untrusted callers.
