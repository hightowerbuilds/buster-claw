...
You run **Claude Code, Codex, or OpenCode** — in the built-in terminal, or headlessly from the app's own Chat. The agent drives the app through one canonical command surface (206 commands) covering your browser, Gmail, Calendar, Drive, GitHub, and a durable work queue. Every command that changes something, every outbound send, and every untrusted fetch lands on an auditable security feed, and restricted actions are refused outright for untrusted callers.
...
- **One command surface.** 206 commands across documents, browser, Google Workspace, integrations, finance, phone, notes, memory, skills, and orchestration — reachable from the CLI and an HTTP API, with per-caller trust tiers and an audit trail covering everything that changes state.
