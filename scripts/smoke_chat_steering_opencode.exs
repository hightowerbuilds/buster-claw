# CHAT_LIVE_STEERING_ROADMAP Phase 4 acceptance: does OpenCode, through
# `Chat.submit/3`, get BOTH of the things it lacked — mid-turn steering and
# conversation continuity?
#
#   mix run scripts/smoke_chat_steering_opencode.exs
#
# The opencode counterpart, completing the three-backend parity check. Opt-in,
# drives the operator's real `opencode`, and costs money.
#
# ⚠ OpenCode is the one backend that cannot always PROVE acceptance:
# `prompt_async` returns an empty body, so the evidence is an SSE echo that may
# not arrive in the window. A `{:ok, :sent}` here is therefore a PASS for
# steering and a note about the receipt, not a failure — the run either changed
# course or it did not, and the tool calls below say which.
#
...
unless System.find_executable("opencode") do
  IO.puts("opencode is not on PATH — this smoke needs the operator's real CLI.")
  System.halt(1)
end

Application.put_env(:buster_claw, :chat_live_steering_enabled, true)

conv = "smoke-opencode-#{System.unique_integer([:positive])}"
Chat.subscribe(conv)
...
IO.puts("\nsession after turn 1: #{inspect(thread)}")

# --- turn 2: continuity ------------------------------------------------------
#
# The gap `opencode exec` left. A second turn must be able to refer to the first;
...
# `:sent` counts: the message WAS delivered, opencode just could not confirm it
# in the window. What proves the steer is the behaviour below, not the label.
steered? = first.steered in [{:ok, :steered}, {:ok, :sent}]
confirmed? = first.steered == {:ok, :steered}
...
IO.puts("\n" <> String.duplicate("=", 68))
IO.puts("VERDICT: #{if pass?, do: "PASS — opencode steers AND remembers", else: "FAIL"}")
IO.puts(String.duplicate("=", 68))
IO.puts("  STEERING")

IO.puts(
  "    Chat.submit returned:  #{inspect(first.steered)}   ({:ok, :steered} or {:ok, :sent})"
)

IO.puts("    acceptance confirmed:  #{confirmed?}   (false = delivered, receipt missed)")
IO.puts("    redirect ran:          #{redirected?}")
IO.puts("    step three skipped:    #{skipped?}")
IO.puts("    turns completed:       #{first.results}   (must be 1)")
IO.puts("  CONTINUITY")
IO.puts("    session id:             #{inspect(thread)}")
IO.puts("    same session on turn 2: #{same_thread?}")
