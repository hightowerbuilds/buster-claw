...
## Wallets — a financial-management tab (ledger + budgets + polling)

New top-level **Wallets** feature: businesses get a transaction ledger, individuals
get a personal wallet with monthly budgets, and either can **poll external sources**
in real time. Built in two phases against the existing patterns (canonical command
catalog, LiveView + PubSub, a `Dispatcher`-style poller). Decisions locked with the
user up front: all three polling sources, transaction-ledger core, agent reads
`:safe` / writes `:restricted` / deletes gated.

- **Phase 1 — model + tab** (`wallets.ex`, `wallets/{wallet,transaction,budget}.ex`,
  3 migrations, `wallets_live.ex`). `wallets`/`wallet_transactions`/`wallet_budgets`;
  money is **integer cents** (no prior money convention; SQLite has no decimal). Each
  wallet caches `balance_cents`, recomputed atomically in a `Repo.transaction` on
  every add/delete. Context mirrors `integrations.ex` (PubSub `"wallets"` topic +
  `broadcast_change`); `budget_summary/2` sums month-scoped actuals vs. targets via
  a `strftime('%Y-%m', occurred_on)` filter. New `/wallets` dock tab + route +
  LiveView: real-time wallet list, create form, detail view with ledger + running
  balance, and a **budget panel for personal wallets**.

- **Phase 2 — polling** (`wallet_poller.ex`, `wallets/feed.ex`, `wallet_feeds`
  migration). `WalletPoller` GenServer modeled on `dispatcher.ex` (self-rescheduling
  tick, crash-safe `safe_tick`, `tick_now/1`, injectable `:finance`/`:fetch`),
  supervised behind `:wallet_poller_enabled` (off in test). Three sources:
  **market** → `Finance.quote` (Finnhub, key-gated → records `last_error` cleanly);
  **url** → SSRF-guarded `Browser.fetch` + `Library.body_hash` change detection;
  **integration** → latest run summary. Plus event-driven **Gmail** receipts: the
  poller subscribes to the Dispatch + Integrations PubSub topics and records inbound
  `source: "gmail"` items against `gmail` feeds (amount extraction deferred). Feed UI
  in the wallet detail view.

- **Command surface** (`commands.ex`). `{:wallet, Wallets, :wallet, :wallets}` added
  to the auto-CRUD loop, plus custom `wallet_list_transactions`/`wallet_add_transaction`/
  `wallet_set_budget`/`wallet_budget_summary` and feed verbs
  `wallet_feed_list`/`create`/`update`/`delete` + `wallet_poll`. Tiers map to the
  guardrails: reads `:safe`, writes `:restricted`, deletes gated — enforced by the
  existing `authorize/2` (no new auth code).

### Wallets verification
- `mix test` — **552 tests, 0 failures** (was 530). New: `wallets_test` (context CRUD,
  balance recompute, month-scoped budget summary, feed validation, PubSub broadcasts),
  `wallet_poller_test` (market tick via injected finance fn, url change-detection
  across two polls, error-records-but-keeps-going, Gmail event → gmail feed, gmail
  excluded from the timer poll), `wallets_live_test` (create/open/ledger/balance,
  personal budget panel, feed add, delete), and wallet/feed tier+gating cases in
  `commands_test`. Clean `--warnings-as-errors`; new files `mix format`-clean.
- **Verified in-process, not over HTTP:** catalog wiring, trust tiers, and gating
  were proven by calling `Commands.call/3` directly in tests rather than the
  `./buster-claw` CLI — booting the server outside test env gets SIGTERM'd as an
  agent task. **Still unverified by me:** the live two-window real-time check in the
  running app (needs `mix phx.server` + `/wallets` in a browser).

## Blocker found — Tauri build fails (`allow-browser-screenshot` not found)

Running `./scripts/dev.sh` to see Wallets in the desktop shell, `cargo tauri dev`
fails in `build.rs`: **`Permission allow-browser-screenshot not found`**. Unrelated
to the Wallets work (all Elixir) — it's a pre-existing capabilities mismatch in
`desktop/tauri`: `capabilities/default.json` lists `allow-browser-screenshot` and
`build.rs` registers the `browser_screenshot` command, but
`permissions/autogenerated/browser_screenshot.toml` is **missing** (every other
browser command has one). Fix is to add that autogenerated TOML (mirroring
`browser_open.toml`). **Not applied yet — paused per request.**
