...
- Next consolidation candidates if we keep going: the `dispatch *` operator
  verbs are still the agent's write-back path (keep), but `shift run` /
  `mailman poll` can be fully removed once muscle-memory has moved to `on-duty`.
