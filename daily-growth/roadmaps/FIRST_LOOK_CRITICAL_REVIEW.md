...
- Favicon fetch only tries `/favicon.ico` and never parses `<link rel="icon">` (`favicons.ex:90`), so many modern sites show a blank favicon — cached for 7 days.
...
- `MAX_LIVE_TABS = 6` (`browser.rs:77`) — the 7th+ tab is evicted and *reloads* on switch-back, silently losing scroll position and **in-progress form input.**
- Find-in-page has no match counts (`browser.rs:530-547`).
- `Search` (DuckDuckGo HTML scraping, `search.ex:4`), `Memory.search` (run-history FTS5), and library filtering all share the "search" name — three unrelated concepts. DDG scraping is fragile and will intermittently return nothing with no clear error.
