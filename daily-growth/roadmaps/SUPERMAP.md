...
| **The corner widget's sky** | `Widget.PlacePanel`, `Appearance` surface `:widget` | **Phases 0–2 SHIPPED 08-15** — `home_widget.ex` decomposed 699 → 135 (FROZEN → HELD), `:widget` is a real surface with `daycycle` as a default nothing can select, and the panel now follows it live. `data-daylight` derives from the SHADER, so daycycle keeps its clock on any surface. Phases 3 (UI density) and 4 (prose) remain | [`WIDGET_BACKGROUND`](surfaces/WIDGET_BACKGROUND_ROADMAP.md) |
